...
// Invoices rejection
export const approveInvoice = async (scanId: string) => {
    ...
    // Only allow approving pending_review items to avoid double rewarding
    if (scanData.status === 'processed') {
        throw new Error("La factura ya fue aprobada");
    }
    ...
    batch.update(scanRef, {
        status: 'processed',
        rejectionReason: deleteField() // remove rejection reason if any
    });

    // 2. Add User Points (if user exists and points were > 0)
    ...
    // 3. Add Pharmacy Stats (if pharmacy exists)
