...
export const processInvoice = functions.firestore
    ...
    .onUpdate(async (change: functions.Change<functions.firestore.QueryDocumentSnapshot>, context: functions.EventContext) => {
        ...
        try {
            ...
            const productsConfigString = products.map((p: any) => {
                let str = `- Name: "${p.name}"`;
                if (p.approxBoxPrice) str += `, BoxPrice: ${p.approxBoxPrice}`;
                if (p.approxUnitPrice) str += `, UnitPrice: ${p.approxUnitPrice}`;
                return str;
            }).join('\n                   ');
            ...
            const repRewards: Record<string, number> = {}; // { repId: points }
            ...
            const reps = repsSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as any));
            ...
            aiData.products.forEach((match: any) => {
                ...
                if (productConfig) {
                    ...
                    validProducts.push({
                        ...
                        quantity,
                        unitPrice,
                        saleType: match.saleType, // Save the saleType from AI
                        commissionPct,
                        points,
                    ...
                    const matchingReps = reps.filter(r =>
                        r.productLines && r.productLines.includes(productLine)
                    );
            ...
            const repUpdates = Object.keys(repRewards).map(repId => {
                ...
                return db.collection('users').doc(repId).update({
                    points: admin.firestore.FieldValue.increment(points), // Assuming Reps also have a points wallet/score
